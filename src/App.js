...
function App() {
  ...
  const OTHERSACTIVITES = [
    {
      _id: 123144324,
      activity: "Desarrollo curricular (proyecto formativo, planeación pedagógica, elaboración de guías de aprendizaje, instrumentos de evaluación y recursos educativos)",
    ...
    {
      _id: 123144324,
      activity: "Aseguramiento de la calidad (Registro calificado, Autoevaluación Institucional)",
      description: "Sin descripción 3",
      duration: 22,
    },
  ];
  ...
  const handleGeneratePdf = () => {
    ...
    doc.html(reportTemplateRef.current, {
      async callback(doc) {
        doc.save(`RMI ${new Date().getFullYear()} ${USER.month} ${USER.name}.pdf`);
      },
    });
  };
  ...
  return (
    <div>
      <button className="button" style={{width: '120px', height: '30px', marginBottom: '10px', background: 'white', border: '1px solid', borderRadius: '7%', cursor: 'pointer'}} onClick={handleGeneratePdf}>
        Generar PDF
      </button>
      <div ref={reportTemplateRef}>
        <ReportTemplate user={USER} rmi={RMI} othersActivities={OTHERSACTIVITES} />
      </div>
    </div>
  );
}
